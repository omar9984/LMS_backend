...
exports.addCourse = catchAsync(async (req,res) => {
    ...
    if (learner.courses.includes(req.params.id)) return res.status(400).json("Already enrolled")
    ...
    let learnerDoc = await User.findByIdAndUpdate({_id: req.user.id}, learnerupdatedData)
    ...
    let courseDoc = await Course.findByIdAndUpdate({_id: req.params.id},courseupdatedData)
...
exports.leaveCourse = catchAsync(async (req,res) => {
    ...
    let learner = await User.findById(req.user.id)
    if (!learner) return res.status(404).json("learner not found!")
    if (!learner.courses.includes(req.params.id)) return res.status(400).json("Already Not Enrolled")
    let learnerupdatedData = {
        $pull:{
            courses:req.params.id
        }

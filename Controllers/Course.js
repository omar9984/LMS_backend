...
exports.addQuestion = catchAsync(async (req, res) => {
  ...
  if (user.type == "learner") {
    if (!course.learners.includes(user._id))
      return res
        .status(401)
        .json("not authorized, Please enroll in the course");
  }
  ...
  if (user.type == "instructor") {
    if (!course.instructor.equals(user._id))
      return res
        .status(401)
        .json("not authorized, Course belongs to another instructor");
  }
...
exports.deleteQuestion = catchAsync(async (req, res) => {
  ...
  if (!course.instructor.equals(user._id) && user.type != "admin")
    return res
      .status(401)
      .json("not authorized, Course belongs to another instructor");
